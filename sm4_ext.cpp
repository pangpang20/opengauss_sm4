...
static int get_iv_bytes(text *iv_text, uint8_t *iv_bytes)
{
    char *iv_str = text_to_cstring(iv_text);
    size_t iv_len = strlen(iv_str);

    if (iv_len == SM4_IV_SIZE) {
        memcpy(iv_bytes, iv_str, SM4_IV_SIZE);
    } else if (iv_len == SM4_IV_SIZE_HEX) {
        size_t bytes_len;
        if (hex_to_bytes(iv_str, SM4_IV_SIZE_HEX, iv_bytes, &bytes_len) != 0) {
            pfree(iv_str);
            return -1;
        }
    } else {
...
sm4_encrypt(PG_FUNCTION_ARGS)
{
    ...
    if (get_key_bytes(key, key_bytes) != 0) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("SM4 key must be %d bytes or %d hex characters", SM4_KEY_SIZE, SM4_KEY_SIZE_HEX)));
    }
    ...
    if (sm4_ecb_encrypt(key_bytes, (uint8_t *)plain_str, plain_len, cipher, &cipher_len) != SM4_SUCCESS) {
        pfree(plain_str);
        pfree(cipher);
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("SM4 ECB encryption failed")));
    }
...
sm4_decrypt(PG_FUNCTION_ARGS)
{
    ...
    if (sm4_ecb_decrypt(key_bytes, cipher, cipher_len, plain, &plain_len) != SM4_SUCCESS) {
        pfree(plain);
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("SM4 ECB decryption failed")));
    }
...
sm4_encrypt_cbc(PG_FUNCTION_ARGS)
{
    ...
    if (get_iv_bytes(iv_text, iv_bytes) != 0) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("SM4 IV must be %d bytes or %d hex characters", SM4_IV_SIZE, SM4_IV_SIZE_HEX)));
    }
    ...
    if (sm4_cbc_encrypt(key_bytes, iv_bytes, (uint8_t *)plain_str, plain_len, cipher, &cipher_len) != SM4_SUCCESS) {
        pfree(plain_str);
        pfree(cipher);
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
...
sm4_decrypt_cbc(PG_FUNCTION_ARGS)
{
    ...
    if (sm4_cbc_decrypt(key_bytes, iv_bytes, cipher, cipher_len, plain, &plain_len) != SM4_SUCCESS) {
        pfree(plain);
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
